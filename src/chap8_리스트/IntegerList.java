...
class LinkedList1 {
	...
	public boolean Delete(int element) { // delete the element
		...
		Node1 current = first, previous = null;
		while (current != null) {
	...
	{
		Node1 nd = new Node1(element); // 삽입할 노드
		Node1 current = first; // 현재 노드
		Node1 previous = null; // 이전 노드
		if (current == null) {// first가 널값인 경우(값이 없을때)
			first = nd;
			return;
		}
