...
class Tree4 {
	...
	TreeNode4 inorderSucc(TreeNode4 current) {// 중위 순회? 이거 무슨 함수인지 모르겠음???
	...
	TreeNode4 findParent(TreeNode4 current, Comparator<? super SimpleObject2> c) {// 부모노드 찾기
	...
	boolean isLeafNode(TreeNode4 current) { // 리프노드인지 확인(가장 마지막?에 있는 노드 ; 노드가 더 이상 뻗어나가지 않는 노드(자식 노드 없음)
	...
	void inorder(TreeNode4 CurrentNode) {// 중위 순회
		if (CurrentNode != null) {
	...
	void preorder(TreeNode4 CurrentNode) {// 전위 순회
		if (CurrentNode != null) {
			System.out.print(CurrentNode.data + " ");
	...
	void postorder(TreeNode4 CurrentNode) { // 후위 순회
		if (CurrentNode != null) {
	...
	public boolean add(SimpleObject2 obj, Comparator<? super SimpleObject2> c) { //노드 추가
		TreeNode4 current = null;
		if()
		return false;
	}
